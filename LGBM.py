...
#     verbose = True
# )
# result = grid_search.fit(x_trainSmote,y_train_smote)
# theBestParameters = result.best_estimator_

#lgb model
print("model start")
model2 = lgb.LGBMClassifier(n_estimators = 600, learning_rate = 0.1, max_depth = 5,objective = "binary",seed = 42 )
model2.fit(x_trainSmote, y_train_smote)
pred = model2.predict(x_test1tf)
print(classification_report(y_true = y_test1, y_pred =pred , digits = 6))

#finish the prediction and create a csv file.
preds = model2.predict(testtf)
